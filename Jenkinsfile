pipeline {
    ...
    environment {
        PROD_ENV     = "/opt/env/.env.gpsmetan"
        IMAGE_NAME   = "gpsmetan"
        TEST_TAG     = "test"
        PROD_TAG     = "latest"
        CONTAINER_DB = "gpsmetan_db_test"
        CONTAINER_WEB = "gpsmetan_web_test"
        CONTAINER_REDIS = "gpsmetan_redis_test"
        STACK_NAME = "gpsmetan"
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', credentialsId: 'ssh', url: 'git@github.com:JscorpTech/gpsmetan.git'
            }
        }
        stage('Prepare') {
            steps {
